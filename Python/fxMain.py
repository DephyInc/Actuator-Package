...
def print_usage(prog_name: str):
	...
	print(
		'\t"connected_devices" ONLY required for specific experiments\n'
...
def get_exp_ind(argv):
	...
	if not exp_ind.isdecimal():  # Filter out letters
		sys.exit("Please choose an experiment between [0 - {} ]".format(len(EXPERIMENTS) - 1))
...
def get_dev_num(argv, exp_ind):
	...
	print("Max number of devices for this experiment: {}".format(EXPERIMENTS[exp_ind][3]))
	# Get it from the command line argument list
	if len(argv) > 2:
		dev_num = argv[2]
	# Or prompt the user for it
	...
	sys.exit(
		"Please choose a number of device between {} and {}".format(
...
def main(argv):
	...
	try:
		if dev_num == 1:
			...
			EXPERIMENTS[exp_ind][0](flex.FlexSEA(), ports[0], baud_rate, ports[1])
		else:
