...
def printDevice(actPackState):
	...
	print('Accel X: ', actPackState.accelx, ', Accel Y: ', actPackState.accely, ' Accel Z: ',
		actPackState.accelz)
	print('Gyro X: ', actPackState.gyrox, ', Gyro Y: ', actPackState.gyroy, ' Gyro Z: ',
		actPackState.gyroz)
	print('Motor angle: ', actPackState.encoderAngle, ', Motor voltage: ',
		actPackState.motorVoltage)
...
def fxTwoPositionControl(port, baudRate, expTime=10, time_step=0.1, delta=10000,
    ...
    for i in range(num_time_steps):
        ...
        sleep(time_step)
        print('Holding position:', positions[current_pos], end='\r')
