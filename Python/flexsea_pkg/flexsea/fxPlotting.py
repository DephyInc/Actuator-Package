...
def plot_setpoint_vs_desired(
        dev_id, fig, ctrl_type, sig_freq, sig_amplitude, sig_type, cmd_freq,
        times, reqs, measurements, cycle_stop_times):
    """
    ...
    if ctrl_type == en.HSS_CURRENT:
        title_str = 'Current control with {:.0f} Hz, {:.1f} mA {} and {:.0f} Hz commands (ID {})'
        title = title_str.format(
            sig_freq, sig_amplitude, sig_type, cmd_freq, dev_id)
        plt.ylabel('Motor current (mA)')

    # Position controller:
    elif ctrl_type == en.HSS_POSITION:
        title_str = 'Position control with {:.0f} Hz, {:.0f} tick{} {} and {:.0f} Hz commands (ID {})'
        title = title_str.format(
            sig_freq, sig_amplitude,
            ('s' if sig_amplitude > 1 else ''),  # Pluralizes tick
            sig_type, cmd_freq, dev_id)
