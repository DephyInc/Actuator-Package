...
class Device:
	...
	def open(self, freq, log_level=4, log_enabled=True):
		...
		"""
		self.dev_id = self.clib.fxOpen(self.port.encode("utf-8"), self.baud_rate, log_level)
		if self.dev_id == -1:
	...
	def read_device(self):
        ...
        # Net master
			...
			ret_code = self.clib.fxReadNetMasterDevice(self.dev_id, c.byref(device_state))
	...
	def read_device_all(self, data_size):
        ...
        # Net master
            ...
            n_read = self.clib.fxReadNetMasterDeviceAll(self.dev_id, c.byref(data), data_size)
    ...
    # set_gains
    # -----
	def set_gains(self, kp, ki, kd, k_val, b_val, ff):
		"""
        ...
        IOError:
            Command failed.
		"""
		ret_code = self.clib.fxSetGains(self.dev_id, kp, ki, kd, k_val, b_val, ff)
	...
	def send_motor_command(self, ctrl_mode, value):
        ...
        IOError:
            Command failed.
		"""
		ret_code = self.clib.fxSendMotorCommand(self.dev_id, ctrl_mode, c.c_int(int(value)))
	...
	def find_poles(self):
		...
		"""
        user_input = input("WARNING: You should not use this function unless you know what "\
            "you are doing!\nProceed?[y/n] ")
        if user_input != "y":
