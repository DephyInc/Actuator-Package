#ifndef XC00S_STRUCT_H
#define XC00S_STRUCT_H
...
#define XC00S_SYSTEM_TIME_POS 60
#define XC00S_STRUCT_DEVICE_FIELD_COUNT 61
#define XC00S_LABEL_MAX_CHAR_LENGTH 19

/// This is The Device fields * 10 + deviceField + 1. Ten is the max string length of 2^32 in
/// decimal separated from commas
#define XC00S_DATA_STRING_LENGTH 672
...
///
/// \param XC00s is the struct with the data to be set
///
...
///
void XC00sSetData(struct XC00sState *xc00s, const uint32_t _deviceStateBuffer[], int systemStartTime);

///
/// \brief takes all data and places it into single, comma separated string
///
/// \param XC00s is the struct with the data to be placed in the string
///
/// \param dataString is where the new string wll be placed
///
void XC00sDataToString(struct XC00sState *xc00s, char dataString[XC00S_DATA_STRING_LENGTH]);
...
///
void XC00sGetLabels(char labels[XC00S_STRUCT_DEVICE_FIELD_COUNT][XC00S_LABEL_MAX_CHAR_LENGTH]);
...
/// \param labels is the array of labels containing the parameter names
///
int XC00sGetLabelsForLog(char labels[XC00S_STRUCT_DEVICE_FIELD_COUNT][XC00S_LABEL_MAX_CHAR_LENGTH]);

///
/// \brief Places data from struct into an array.
///
/// \param xc00s the data to be converted to an array
///
/// \param xc00sDataArray the array in which to place the data
///
void XC00sStructToDataArray(struct XC00sState xc00s, int32_t xc00sDataArray[XC00S_STRUCT_DEVICE_FIELD_COUNT]);

///
/// \brief Get data based on data position from device communication.
///
/// \param xc00s the data to access
///
/// \param dataPosition the position of data to access
///
/// \param dataValid return false if requested data position is invalid
///
int GetXC00sDataByDataPosition( struct XC00sState xc00s, int dataPosition);

#ifdef __cplusplus
} //extern "C"
#endif

#endif //XC00S_STRUCT_H
